...
- **4 Spiele:**
  ...
  - **Katapult Tower (1v1)** — 1v1 per Invite, rundenbasiert wie Tic-Tac-Toe (kein High-Frequency-Sync nötig, weil immer nur einer gleichzeitig schießt). Zwei Katapulte gegenüber, 3 Hindernisse in der Mitte (je 1 Treffer zerstörbar), wer als erstes die Gegner-Figur trifft gewinnt. Flugbahn läuft lokal beim Schützen; der wartende Spieler bekommt die Schuss-Parameter (`lastShot`) und spielt exakt die gleiche Flugbahn nach, damit beide das Gleiche sehen
  - **Snake** — Solo-Arcade, kein Invite nötig, eigener Highscore + globales Leaderboard
- Solo-Spiele (aktuell nur Snake) haben ein Highscore-Leaderboard (`scores`-Collection) — async "Wer ist am besten", kein Echtzeit-Risiko
