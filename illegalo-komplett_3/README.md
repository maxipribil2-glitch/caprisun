...
- Kill-Switch — eigenständig (`gc-admin.html`), integriert ins Illegalo-`admin.html` **und** ins Illegalo-`dev.html` (Tab "Steuerung") ✅
...
1. Registrier dich ganz normal über `gc-index.html` mit deinem eigenen Account (oder einem dedizierten Service-Account, z.B. `illegaloadmin`, wenn du den Kill-Switch übers Illegalo-`admin.html`/`dev.html` steuern willst)
...
4. Fertig — jetzt siehst du in der Lobby unten rechts den `admin` Link (zu `gc-admin.html`), und falls du den Service-Account-Weg gewählt hast: trag die **gleichen** Zugangsdaten sowohl in Illegalos `admin.html` als auch in `dev.html` bei `GC_SERVICE_EMAIL`/`GC_SERVICE_PASSWORD` ein (zwei separate Stellen, gleicher Account)

Wenn du auf **SERVER AUSSCHALTEN** klickst, sehen alle anderen Besucher (egal ob eingeloggt oder auf der Login-Seite) sofort live "Sorry, die Server sind gerade aus." — du selbst kommst über `gc-admin.html` (oder Illegalos `admin.html`/`dev.html`) trotzdem immer rein, um's wieder einzuschalten.
