...
-- use_bonus_spin/buy_bonus_spin reason === 'killswitch_active' ab (mit passendem
-- "Server im Wartungsmodus"-Hinweis), aber KEINE dieser RPCs hat den Kill Switch
-- je serverseitig geprüft — der Kill Switch griff bisher nur in firestore.rules,
-- nicht hier. Gleiches Prinzip wie killSwitchActive() dort: Staff darf durch
-- (braucht Zugriff aufs Dev Panel um ihn selbst wieder auszuschalten).
